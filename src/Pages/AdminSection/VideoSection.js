...
const VideoSection = () => {
    ...
    useEffect(() => {
        fetch(`https://interiordesignseoclone-610277fa395f.herokuapp.com/videos`)
            .then((res) => res.json())
            .then((info) => setVideo(info));
    }, []);

    useEffect(() => {
        fetch(`https://interiordesignseoclone-610277fa395f.herokuapp.com/videos-items`)
            .then((res) => res.json())
    ...
    const handleVideoSection = (event) => {
        ...
        const url = `https://interiordesignseoclone-610277fa395f.herokuapp.com/add-video`;
        fetch(url, {
            method: "POST",
            headers: {
    ...
    const handleAddItem = (event) => {
        ...
        const url = `https://interiordesignseoclone-610277fa395f.herokuapp.com/add-video-item`;
        fetch(url, {
            method: "POST",
            headers: {
                "content-type": "application/json",
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-8 col-md-12">
                                <h2 className='mb-5 mt-5'> Update Video Section </h2>
                                {
                                    video.length === 1 &&
                                    <> {
                                        video.map(s => <Link className='thm-btn' to={`/videotext-edit/${s._id}`}>Update Video</Link>)
                                    }</>
                                    ...
                                    <form class="contact-form " onSubmit={handleVideoSection}>
                                        <div class="row">
                                            ...
                                            <div class="slider-btn">
                                                <button className="thm-btn" data-animation="fadeInRight" data-delay=".8s"> Update Video Section </button>
                                            </div>
                                        </div>
                                ...
                                <form class="contact-form" onSubmit={handleAddItem}>
                                    <div class="row">
                                        ...
                                        <div class="slider-btn">
                                            <button className="thm-btn" data-animation="fadeInRight" data-delay=".8s"> Add Team Member </button>
                                        </div>
                                    </div>
